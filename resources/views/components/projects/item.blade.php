...
<div x-data="{ open:false }" class="relative">
    <div @click="open = !open" class="block w-full p-4 border border-slate-200 rounded-md shadow bg-white hover:border-slate-300 hover:shadow-lg cursor-pointer transition-colors transition-shadow mb-4" wire:key="{{ $project->id }}">
        <div class="flex items-center">
            ...
            <div>
                <div class="mb-0.5 text-slate-400">
                    <span class="text-slate-700 font-medium">{{ $project->name }}</span>
                    <span class="text-xs">{{ '@' . $project->slug }}</span>
                </div>
    ...
    <div x-show="open" x-on:click.outside="open = false" class="absolute z-30 top-full left-0 mt-2 py-2 rounded bg-white border border-slate-200 shadow">
        ...
        @endif
        <a href="{{ route('projects.show', ['slug' => $project->slug]) }}" target="_blank" class="w-full text-slate-500 hover:text-slate-600 hover:bg-slate-100 flex items-center gap-x-2 py-2.5 px-4 text-sm cursor-pointer transition-colors border-b border-slate-200">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
