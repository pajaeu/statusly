<div>
    <div class="fixed top-0 bottom-0 right-0 z-50 mt-3 mr-3 flex flex-col">
        @foreach($messages as $message)
            <div wire-key="{{ uniqid() }}" x-data="{ closed: false }" x-show="!closed" x-init="setTimeout(() => closed = true, 3000)" x-transition class="mb-4 py-3 px-4 flex items-center gap-2 font-medium text-slate-900 bg-slate-200 rounded-lg">
